<div class="space-y-6" wire:poll.15s>
    ...
    >
        <x-slot name="actions">
            <a href="{{ route('parking.map') }}" wire:navigate class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
                <svg class="h-4 w-4 text-red-600" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-width="1.5" stroke-linecap="round" d="M9 20.25 3.75 18V5.25L9 7.5l6-2.25 5.25 2.25V18L15 20.25 9 18l-6 2.25Z"/></svg>
                {{ __('Buka denah') }}
            </a>
        </x-slot>
    </x-hotel.page-header>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        @foreach ($zones as $zone)
            <div class="vuexy-card p-4">
                <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">{{ $zone['label'] }}</p>
                <p class="mt-2 text-2xl font-bold text-slate-800 dark:text-white">{{ $zone['available'] }}<span class="text-sm font-normal text-slate-400"> / {{ $zone['capacity'] }}</span></p>
                <p class="mt-1 text-xs text-slate-500">{{ __('Kosong · Terisi :o · Booking :r', ['o' => $zone['occupied'], 'r' => $zone['reserved']]) }}</p>
                <a href="{{ route('parking.map', ['area' => $zone['key']]) }}" wire:navigate class="mt-3 inline-block text-xs font-semibold text-red-600 hover:text-red-700">{{ __('Buka denah') }} →</a>
            </div>
        @endforeach
    </div>
            ...
            <div class="flex h-64 items-end gap-1">
                @foreach ($hourly as $hour => $count)
                    ...
                    <div class="group flex min-w-0 flex-1 flex-col items-center justify-end gap-2">
                        <div
                            class="w-full max-w-[22px] rounded-t-lg bg-gradient-to-t from-red-600 to-red-400 opacity-90 shadow-sm transition-all duration-300 group-hover:opacity-100"
        ...
        <x-hotel.card :title="__('Kapasitas parkir')">
            <div class="space-y-5">
                <div class="flex items-center justify-between text-sm">
                    ...
                    <span class="text-lg font-bold text-red-600 dark:text-red-400">{{ $utilization }}%</span>
                </div>
                <div class="h-2.5 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                    <div class="h-full rounded-full bg-gradient-to-r from-emerald-500 via-amber-400 to-rose-500 transition-all duration-500" style="width: {{ min(100, $utilization) }}%"></div>
                </div>
    ...
    <x-hotel.card :title="__('Aktivitas terbaru')">
        <x-slot name="header">
            ...
            <a href="{{ route('parking.map') }}" wire:navigate class="text-sm font-medium text-red-600 hover:text-red-700 dark:text-red-400">{{ __('Lihat denah') }} →</a>
        </x-slot>
            ...
            @forelse ($recent as $tx)
                <li class="flex items-center justify-between gap-4 py-3.5 first:pt-0 last:pb-0">
                    <div class="flex items-center gap-3">
                        <span class="flex h-10 w-10 items-center justify-center rounded-full bg-red-500/10 text-red-600 dark:text-red-400">
