<div class="space-y-6" wire:poll.10s>
    ...
    >
        <x-slot name="actions">
            <a href="{{ route('parking.history.excel', request()->query()) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-emerald-700">
    ...
    <x-hotel.card :title="__('Filter')">
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <input type="date" wire:model.live="date_from" class="w-full rounded-lg border-slate-200 bg-slate-50 text-sm focus:border-red-500 focus:ring-red-500/20 dark:border-slate-600 dark:bg-slate-800" />
            </div>
            <div>
                <label class="mb-1.5 block text-xs font-medium text-slate-600 dark:text-slate-400">{{ __('Sampai tanggal') }}</label>
                <input type="date" wire:model.live="date_to" class="w-full rounded-lg border-slate-200 bg-slate-50 text-sm focus:border-red-500 focus:ring-red-500/20 dark:border-slate-600 dark:bg-slate-800" />
            </div>
            <div>
                <label class="mb-1.5 block text-xs font-medium text-slate-600 dark:text-slate-400">{{ __('Status') }}</label>
                <select wire:model.live="status" class="w-full rounded-lg border-slate-200 bg-slate-50 text-sm focus:border-red-500 focus:ring-red-500/20 dark:border-slate-600 dark:bg-slate-800">
                    ...
                    @forelse ($rows as $row)
                        <tr class="transition hover:bg-slate-50/80 dark:hover:bg-slate-800/30">
                            ...
                            <td class="px-6 py-3.5">
                                <span class="rounded-md bg-red-500/10 px-2 py-0.5 text-xs font-semibold text-red-700 dark:text-red-300">{{ $row->slot?->slot_code }}</span>
                            </td>
